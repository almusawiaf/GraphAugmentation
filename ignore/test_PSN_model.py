...
df = pd.read_csv(f'data/{DB[2]}', skiprows=skipping[d])
...
df2 = df.head(50)
G1 = nx.Graph()
G1.add_nodes_from(list(df2['vha_id']))
Nodes = list(G1.nodes())
...
def batch_processing(N):
    results = []
    for u,v in N:    
        vector1 = df2[df2['vha_id']==v].drop('vha_id', axis=1).to_numpy().tolist()[0]
        vector2 = df2[df2['vha_id']==u].drop('vha_id', axis=1).to_numpy().tolist()[0]
        d = distance.euclidean(vector1,vector2) 
        results.append([u, v, d])
    return results
